...
const getVerdictScore = (verdict: "yes" | "no" | "unsure") => {
  if (verdict === "yes") {
    return 1;
  }
...
export const AnswerRelevancy = createScorer<string, string, string>({
  ...
  scorer: async ({ input, output }) => {
    if (!output || output.trim().length === 0) {
      return { score: 0, metadata: { statements: [], verdicts: [] } };
    }
    ...
    const { object: verdictResult } = await generateObject({
      ...
      prompt: generateEvaluatePrompt({ input, statements }),
      schema: z.object({
        verdicts: z.array(
          z.object({
    ...
    const totalScore = verdicts.reduce(
      (total, verdict) => total + getVerdictScore(verdict.verdict),
      0,
    );
    const score = verdicts.length > 0 ? totalScore / verdicts.length : 0;
